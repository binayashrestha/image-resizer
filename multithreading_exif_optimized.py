...
class ImageResizer:
    ...
    def resize_image(self, image_path: str):
        ...
        try:
            # Load the original image using PIL to preserve EXIF data
            ...
            # Convert resized CV2 image back to PIL format for saving with EXIF data
            resized_image = cv2.cvtColor(resized_image, cv2.COLOR_BGR2RGB)
            pil_resized_image = Image.fromarray(resized_image)

            self.save_image(pil_resized_image, output_path, exif_data)
        except Exception as e:
            logging.error(f"Failed to process image: {os.path.basename(image_path)}. Error: {e}")

    def save_image(self, pil_image, file_path, exif_data):
        if exif_data:
            pil_image.save(file_path, exif=exif_data, quality=94)
        else:
            pil_image.save(file_path, quality=94)
...
if __name__ == "__main__":
    ...
    resizer = ImageResizer(source_dir, destination_dir, target_file_size_mb=0.7, num_threads=3)
